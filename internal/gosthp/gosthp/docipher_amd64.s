...
// Encrypts block.
		...
		LEAQ	·LS_enc_lookup(SB), DX		// Cipher matrix base address.
...
// (register X0) and XOR of plain text block at the end.

		MOVOU	nonce+8(SP), X0
		MOVOU	pt+24(SP), X5
		
		LEAQ	rk+40(SP), SI
		LEAQ	·LS_enc_lookup(SB), DX
		MOVQ	DX, CX
		XORQ	DI, DI
		MOVQ	$0x1000, R11
...
L3:		NOP
		...
		PXOR	X1, X0						
		SUBQ	$16, SI
		LEAQ	·Pi_inverse_table(SB), DX	// Get inversed Pi table address.
