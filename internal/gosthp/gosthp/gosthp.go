...
// ~=  GOSThopper/24  =~
//
// Implementation of block cipher Kuznyechik, GOST R 34.12-2015, with
// assembly optimizations for arm64 and amd64.
//
// Author: Alexander Venedioukhin
// Date: 02/01/2024
// Site/contacts: https://dxdt.ru/
// (Un)Licence: free software, distribution unlimited.
//
// Supplementary files:
//		docipher.go
//		docipher_amd64.go
...
// National Standard).
//
// This is optimized implementation in Golang using lookup
// tables and assembly language using 128-bit machinery on
// x64/amd64 (AVX needed) and arm64 platforms.
// So, on these particular platforms it runs fast.
// For other platforms - there is universal implementation in more
// or less pure Go included. It still employs lookup tables but,
// nevertheless, will be orders of magnitude slower.
//
// See docipher_amd64.s for amd64 assembly and docipher_arm64.s for ARM.
//
// This package implements standard interface for crypto/cipher package
// in Golang, so is compatible with standard GCM library (and with
// others).
//
//
// General usage:
// c, err := gosthp.NewCipher(key) - creates and initializes new instance,
// creates tables and returns cipher.Block c with Kuznyechik;
// c.Encrypt(dst, src), c.Decrypt(dst, src) - block encryption
// and decryption methods;
// DoEncrypt(block, round_keys) - cipher encrypt procedure,
// low level;
// DoDecrypt(block, round_keys) - cipher decrypt procedure,
// low level.
//
// There are also simple counter mode functions:
//
// CM_Encrypt(nonce_iv, key, plain_text);
// CM_Decrypt(nonce_iv, key, cipher_text);
//
...
// encryption.
//
// It is possible to use cipher in GCM mode of operation:
// ---
// import (
//		"gosthp"
//		"crypto/cipher"
// )
//
// kCipher, err := gosthp.NewCipher(key)
...
//
// Other functions:
// InitCipher() - initializes (computes values for) in-memory
// lookup tables needed for encryption/decryption;
//
// More examples:
// ---
// gosthp.InitCipher()
// RoundKeys := gosthp.StretchKey(MainKey)
// CipherText := gosthp.DoEncrypt(PlainText, RoundKeys)
// DecRoundKeys := gosthp.GetDecryptRoundKeys(RoundKeys)
// PlainText := gosthp.DoDecrypt(CipherText, DecryptRoundKeys)
// ---
//
// Note:
// Assembly routines here make use of some instructions for 128-bit
// arithmetic. These may be unavailable in particular architectures,
// despite such architectures being functional amd64 or arm64.
// It means that compiled code, if run with no extra checks, will
// just fail on such systems/architectures ("illegal instruction").
//
//
// Reference:
...
var Pi_inverse_table_ptr = &Pi_inverse_table
...
// Stretches main key (256 bits) to 10 round keys K_1...K_10
// (128 bits each).
// It's essentially a Feistel cipher.
func StretchKey(key [32]uint8) [10][16]uint8 {
	var i, k int
	var C, x, y, z [16]uint8
	var rkeys [10][16]uint8

	// First - split key into pair of subkeys (K_1 = x, K_2 = y).
	for i = 0; i < 16; i++ {
		x[i] = key[i]
		y[i] = key[i+16]
	}
...
// For fast operation (see DoDecrypt()/DoEncrypt()) round keys need
// to be L-inversed (except the K_0) as this allows use of in-memory
// lookup tables. This function implements inversion.
...
// This function stretches keys every time, for faster version
// see (c *GOSThopper) Encrypt().
...
func InitCipher() {
	...
	for i = 0; i < 16; i++ { // 16 bytes.
		for j = 0; j < 256; j++ { // 256 possible values of bytes - used as index.
			...
			// LS transformation (S, then L) as used in encryption.
			LS_enc_lookup[i][j] = x
			...
			x = L_inv(x)
			// SL-inverse transformation as used in decryption.
			SL_dec_lookup[i][j] = x
...
// With crypto/cipher we need to create cipher.Block to pass as
// a block cipher to GCM mode routines (see gostrun.go for examples).
